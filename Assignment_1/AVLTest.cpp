...
bool testOne(){
    ...
    try{
        ...
        std::cout<<"Initializing new Tree"<<std::endl;
        
        if(myBST->size()!=0){//no records
            testResult=false;
...
bool testTwo(){
    ...
    try{
        ...
        myBST->put(myProd);
        if(myBST->size()!=3){//must have 3 records
            testResult=false;
...
bool testThree(){
    ...
    try{
        ...
        myProd = new Product(4);//put with key 4
        myBST->put(myProd);
        if(myBST->size()!=5){//must have 3 records
            testResult=false;
...
bool testSeven(){
    ...
    try{
        ...
        for(int i = 0;i<1000;i++){//do a bunch of insertions (1000)
            myProd = new Product(i);//put with key i
            myBST->put(myProd);
        }
        if(myBST->size()==1000 && !myBST->isEmpty()){//size is 1000 and is not empty
            std::cout<<"PASSES"<<std::endl;
            testResult=true;
            return true;
        }
...
bool testNine(){
    ...
    try{
        ...
        if(myBST->get(2)->getKey()==2 && myBST->get(2)->getName()=="name1"){//must update name, if exists!
            testResult=true;
            std::cout<<"PASSES"<<std::endl;
            return true;
        }
...
void runTests(int testStart){
    ...
    if(testStart<=1){//run test 1
        testOne();//worth 0.5 marks
    }
    if(testStart<=2){//run test 2
        testTwo();
    }
    if(testStart<=3){//run test 3
        testThree();
    }
    if(testStart<=4){//run test 4
        testFour();
    }
    if(testStart<=5){//run test 5
        testFive();
    }
    if(testStart<=6){//run test 6
        testSix();
    }
    if(testStart<=7){//run test 7
        testSeven();//0.5 marks
    }
    if(testStart<=8){//run test 8
        testEight();//0.5 marks
    }
    if(testStart<=9){//run test 9
        testNine();//0.5  marks
    }
    if(testStart<=10){//run test 10
        testTen();//0.5 marks
    }
    if(testStart<=11){//run test 11
        testEleven();//0.5 marks
    }
    if(testStart<=12){//run test 12
        testTwelve();//0.5 marks
    }
    if(testStart<=13){//run test 13
        testThirteen();//0.5 marks
    }
    if(testStart<=14){//run test 14
        testFourteen();//worth 0.5 marks
    }
    if(testStart<=15){//run test 15
...
int main(int argc, char* argv[]){
    ...
    */
    ...
    std::cout<<"The highest quantity product is: "<<myBST->highestQuantity()->getKey()<<std::endl;
    ...
    */
    ...
    std::cout<<"The product with the smallest key "<<smallest->getKey()<<" is named..."<<smallest->getName()<<std::endl;
    std::cout<<"The highest quantity product is: "<<myAVL->highestQuantity()->getKey()<<std::endl;
